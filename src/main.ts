...
function find_park_hours(park_data:any):string[]
{
  ...
  for(let item of park_data["liveData"])
  {
    if(item["entityType"]=="ATTRACTION")
    {
      ...
      const opening_time_written=opening_time.toLocaleString('en-US', estOptions);
      ...
      const closing_time_written=closing_time.toLocaleString('en-US', estOptions);
  ...
  for(let item of park_data["liveData"])
  {
    if(item["entityType"]=="SHOW"&&!item["name"].includes("Meet"))
    {
      if(item["showtimes"]&&item["showtimes"].length>0&&item["showtimes"].length<=3)
      {
        ...
        const first_opening_hour=first_opening_time.toLocaleTimeString("en",{timeZone:'America/New_York',hour:'numeric',hour12:false});
